...
<meta name="apple-mobile-web-app-title" content="Квест">
...
.counter-block {
  ...
  background:var(--gold-dim); border:1px solid rgba(240,192,80,0.25);
...
.counter-btn {
  ...
  border:1px solid var(--pink); background:var(--pink-glow);
  color:var(--pink); font-size:1.6rem; line-height:1; cursor:pointer;
  -webkit-tap-highlight-color:transparent;
}
...
.counter-value {
  ...
  color:var(--gold); min-width:2ch;
}
.counter-sum { margin-top:10px; font-size:0.78rem; color:var(--text-dim); }

/* === VIDEO === */
.video-block { margin:12px 0; text-align:center; }
.video-btn {
...
<div class="intro" id="intro">
  ...
  <div class="intro-content">
    ...
    <div class="intro-rules">
      ...
      <p>Зманеты трать в магазине. Что там лежит, увидишь сама.</p>
      <p>Один лот откроется только ближе к финалу.</p>
...
const STAGES = [
  ...
  },
  {
    ...
    story:"Это культовый двор. Но <strong>легендарная история о пуховике</strong> случилась не здесь. Ты помнишь. Все помнят.",
  ...
  },
  {
    ...
    story:"У тебя есть фото на шпагате в самых неожиданных местах — в бане на Юшкова, с незнакомыми девушками...",
    ...
    type:"counter", stars:1, counterMax:10
  },
  {
  ...
  },
  {
    ...
    task:"Распиваем страйк или блейзер. Фото в стиле 2010-х: серьёзные лица, чёлки, рок-позы! Конкурс на самую эмо-позу — невеста судит, проигравшие пьют штрафную.",
    type:"simple", stars:1
  },
  {
    emoji:"🍕", name:"КМЛ",
...
const SHOP_ITEMS = [
  ...
  { id:'cool', emoji:'🌬️', name:'Охлаждение', desc:'Вентилятор и вода. Спасает, когда жарко и голова кипит', cost:2 },
...
function loadState() {
  // Сохранения с прошлых версий могут не знать новых полей — дополняем значениями по умолчанию
  try {
    const s=localStorage.getItem('tquest_v4');
    if(s) state=Object.assign({}, state, JSON.parse(s));
  } catch(e) {}
}
function saveState() {
  try { localStorage.setItem('tquest_v4', JSON.stringify(state)); } catch(e) {}
}
...
function getEarnedStars(s,i) {
  ...
  if(s.type==='counter') earned+=state.splitCount;
  return earned;
}
...
function renderTask(s,i) {
  ...
  else if(s.type==='counter') extra=renderCounter(s,i);
...
function renderCounter(s,i) {
  const n=state.splitCount, max=s.counterMax||10;
  const locked=state.completed.includes(i);
  return \`<div class="counter-block">
    <div class="counter-label">Названо мест</div>
    <div class="counter-row">
      <button class="counter-btn" \${locked||n<=0?'disabled':''} onclick="bumpCount(-1,\${max})">−</button>
      <div class="counter-value">\${n}</div>
      <button class="counter-btn" \${locked||n>=max?'disabled':''} onclick="bumpCount(1,\${max})">+</button>
    </div>
    <div class="counter-sum">\${n?'+'+n+' '+zmanetWord(n)+' к награде':'Жми плюс за каждое место'}</div>
  </div>\`;
}

function bumpCount(d,max) {
  const n=state.splitCount+d;
  if(n<0||n>max) return;
  state.splitCount=n;
  saveState();
  renderStages();
...
function renderVideo(s) {
  return \`<div class="video-block">
    <button class="video-btn" onclick="playVideo(this,'\${s.video}')">▶ Включить караоке 🎤</button>
    <div class="video-label">\${s.videoLabel||''}</div>
  </div>\`;
}

function playVideo(btn, src) {
  const box=btn.parentElement;
  box.innerHTML='<video class="video-player" src="'+src+'" controls autoplay playsinline></video>';
}
...
function completeStage(i) {
  ...
  if(s.type==='counter') bonus=state.splitCount;
